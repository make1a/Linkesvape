...
@interface HNNoticeInfoView ()
@property (nonatomic,strong)UIView * lineView;
@property (nonatomic,strong)UIView * containerView;
@end
@implementation HNNoticeInfoView
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)masLayoutSubviews{
    ...
    [self.lineView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(self.detailLabel);
...
- (UILabel *)titleLabel
{
    if(!_titleLabel)
    {
        ...
        _titleLabel.text = NSLocalizedString(@"确定连接设备", nil);
    }
    return _titleLabel;
}
- (UILabel *)detailLabel
{
    if(!_detailLabel)
    {
        ...
        [_detailLabel setFont:[UIFont boldSystemFontOfSize:Handle_width(32/2)]];
        [_detailLabel setTextColor:[UIColor colorWithHexString:@"333333"]];
        _detailLabel.text = @"修改设备名称";
    }
    return _detailLabel;
}
...
- (UIButton *)sumbitButton
{
    if(!_sumbitButton)
    {
        ...
        [_sumbitButton addTarget:self action:@selector(clickRightButton:) forControlEvents:UIControlEventTouchUpInside];
